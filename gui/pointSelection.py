...
class PointSelection(QWidget):
    ...
    def setImage(self):
        ...
        if ret == True:
            ...
            frame = self.video.resizeVideo(
                frame, (self.resizedW, self.resizedH))

            # ... and create the QImage
            qImage = QImage(frame.data, self.resizedW, self.resizedH,
                            3 * self.resizedW, QImage.Format.Format_RGB888).rgbSwapped()

            # Update fields

            emitW = self.resizedW + 125
            emitH = self.resizedH + 10
            # Emit the signal to change the window size to the dimension of the frame
            self.geometryChanged.emit(
                QRect(0, 0, emitW, emitH))
    ...
    def setStartBtn(self, dstPage: Relighting):
        self.startBtn = self.findChild(QPushButton, 'startBtn')
        self.startBtn.clicked.connect(lambda: self.startLightFrameExtraction(dstPage))
    ...
    def getPoints(self, event):
        ...
        needsResize = videoNeedsResize(
            self.video, self.params.getFrameDefaultSize(camOrientation))
    ...
    def startLightFrameExtraction(self, dstPage: Relighting):
        ...
        self.worldHomography = self.videoAnalysis.getWorldHomographyFromPts(
            self.points, int(self.params.getOutputImageSize()))
        ...
        videoSynch = VideoSynchronisation(
            self.params.getStCamVideoPath(), self.params.getMvCamVideoPath())
        ...
        if videoOffset > 0:
            ...
            print(
                f"Static video is shifted. The frame difference is: {abs(frameDiff)}")
        else:
            ...
            print(
                f"Moving video is shifted. The frame difference is: {abs(frameDiff)}")
        ...
        while videoStatic.isOpen() and videoMoving.isOpen():
            ...
            checkStaticFrame = (staticFrame is None or np.shape(
                staticFrame) == () or np.sum(staticFrame) == 0)
            checkMovingFrame = (movingFrame is None or np.shape(
                movingFrame) == () or np.sum(movingFrame) == 0)
        ...
        if (stCamSize is not None):
            # Resize the static frame
            self.firstStaticFrame = cv.resize(
                self.firstStaticFrame, (stCamSize))

        # Extract features from first frame
        featuresFirstStaticFrame = self.videoAnalysis.extractFeaturesFromFrame(
            self.firstStaticFrame, 0)

        for i in range(len(staticFrames)):
            # For each static frame, calculate its features
            pool.add_task(
                self.videoAnalysis.extractFeaturesFromFrame, staticFrames[i], i)
        ...
        for i in range(len(movingFrames)):
            # For each static frame, calculate its features
            pool.add_task(
                self.videoAnalysis.extractFeaturesFromFrame, movingFrames[i], i)
        ...
        featuresFirstFrame = [featuresFirstStaticFrame] * \
            len(featuresStaticFrames)

        # Create a list of features that will be matched using feature matching technique
        featuresStaticStatic = list(
            zip(featuresStaticFrames, featuresFirstFrame))
        featuresStaticMoving = list(
            zip(featuresStaticFrames, featuresMovingFrames))
        ...
        for i in range(len(matchingStaticStatic)):
            ...
            pool.add_task(self.videoAnalysis.getLight, staticFrame, movingFrame,
                          homographyStatic, dstPts, homographyMoving, self.worldHomography, self.kMoving)

        # Wait completion of the queue, and get the results
        pool.wait_completion()
        lightFramePair = pool.get_results()

        validPairs = [pair for pair in lightFramePair if all(
        ...
        sumUV = np.zeros((
            self.params.getOutputImageSize(),
            self.params.getOutputImageSize(),
            2
        ))
        ...
        for worldFrame, light in validPairs:
            ...
            light = np.tile(light[:2].flatten(
            ), (self.params.getOutputImageSize(), self.params.getOutputImageSize(), 1))
        ...
        try:
            ...
            os.mkdir(BASE_DIR)
            self.params.setRelightingBasePath(BASE_DIR)
        except:
        ...
        print("Applying PCA...")
        # Create PCA Class where first reads the stored dataset
        pca = PCAClass(BASE_DIR, fileName, self.params.getPCANumber())
        pca.readDataset()
        # ... and then apply PCA to create a low-dimension vector with data
        pca.applyPCA()
        print("Applying PCA: DONE")

        print("Executing NN training...")
        # After the creation of the low-dimension vector, we can start with NN, first extracting new data.
        nn = NeuralNetwork(BASE_DIR)
        nn.extractDatasets()
        # Then shuffling the dataset before starting the training
        nn.shuffleDataset()
        print("Shuffling: DONE")
        # Lastly, executing the training of the model with current data
        nn.executeTraining()
        # ... and extract mean UV, removing data file (since not required anymore)
        nn.extractUVMean()
